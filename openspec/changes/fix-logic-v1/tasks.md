...
- [ ] **T1** Añadir `"lead_support"` a `_NO_CHOICE_ROLES` (A1+E4)
  - File: `team_generator.py:106-111`
  - Test: `test_choice_item_not_assigned_to_lead_support`

- [ ] **T2** Corregir `assign_role`: comparar `max(atk, spa)` cuando ambos >= 100 (B1)
...
- [ ] **T3** Remover `"Loaded Dice"` de `_BACKUP_ITEMS` (A2)
...
- [ ] **T4** Añadir `redirect` a `role_sp_templates.json` con `{"hp": 32, "spd": 32, "def": 2}` (A3)
  - File: `pokemon_team_builder/data/role_sp_templates.json`
  - Test: `test_suggest_sp_redirect_uses_bulk_template`

- [ ] **T5** Refactorizar `_item_is_activatable` con tabla `_ITEM_PRECONDITIONS` (B3+E7)
...
- [ ] **T6** Corregir `_fallback_move` — nunca retornar move en `used` (E1)
...
- [ ] **T7** Corregir guardias de pass 0 en slot-2 y slot-3 para `cand_cat == ""` (E3)
...
- [ ] **T8** Verificar Protect en move_pool antes de asignar slot 1 (B4)
...
- [ ] **T9** Derivar nature desde slot-2 STAB category (E9, B1 extension)
...
- [ ] **T10** Añadir `_SPECIES_OVERRIDES` y aplicar en `_format_species` (E8)
...
- [ ] **T11** Eliminar penalización de Life Orb en `_items_points` + añadir comment (E5)
...
- [ ] **T12** Añadir docstring a `analyze_coverage` documentando limitación (E6)
...
- [ ] `python -m pytest tests/ -q` → 100% passing (o más, con nuevos tests)
- [ ] Ningún equipo generado con ancla del pool de 167 emite moves duplicados
- [ ] Hydreigon con ancla recibe `special_sweeper` como rol primario
- [ ] Pelipper como lead_support recibe `Timid` o `Calm`, no `Jolly`
- [ ] Un physical_sweeper con Dragon Dance no recibe Weakness Policy
- [ ] kommo-o como ancla exporta `"Kommo-o"` en PokePaste
